...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    for (float depth = 0.0; depth < 100000.0; depth += 2000.0)
    {
        vec3 ray = campos + fragAt * depth;
        
        float alpha = smoothstep(0.5, 1.0, fbm(ray * 0.00003));
        vec3 localcolor = damain_warping_coloring(ray);
        //vec3 localcolor = vec3(1.0,1.0,1.0);
        localcolor = mix(localcolor, vec3(0.3, 0.3, 0.2), alpha);
