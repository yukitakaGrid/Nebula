...
float map(vec3 p){
    ...
    float f = fbm(p-vec3(0,0.5,1.0)*iTime*0.1);
    
    float sph = 1.0-length(p*vec3(0.5,0.8,.5))+f*3.5;
    
    return min(max(0.0,sph),1.0);
    
}

float damain_warping3( in vec3 p , out vec3 q, out vec3 r ){
    p *= 1.5;
    q.x = fbm( p + vec3(0.0,0.0,0.0) );
    q.y = fbm( p + vec3(5.2,1.3,1.0) );
    q.z = fbm( p + vec3(1.0,1.0,1.0) );
    q.x += cos(iTime*0.5)*1.0 + sin(0.3+iTime*0.5)*1.0;
...
vec3 damain_warping_coloring( in vec3 p ){
    ...
    float f = damain_warping3(p,o,n);

    vec3 col = vec3(0.2,0.1,0.9);
    col = mix( col, vec3(0.2,0.0,0.7), 1.5*f );
    col = mix( col, vec3(0.0,0.0,0.0), 1.3*dot(n,n) );
    col = mix( col, vec3(0.6,0.2,0.2), 1.1*o.y*o.y );
    col = mix( col, vec3(0.0,0.0,0.25), 1.3*smoothstep(1.2,1.3,abs(n.y)+abs(n.z)) );
